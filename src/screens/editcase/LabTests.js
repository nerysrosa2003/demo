...
export default class LabTests extends Component {
    ...
    constructor(props){
		...
		this.state = {
            ...
            labs_list: Global.edit_case_json.labs,
    ...
    select_right_menu = (index) => {
        var labs_list = this.state.labs_list;
        for(i = 0; i < labs_list.length; i ++) {
            if(i == index) {
                labs_list[i].clicked = !labs_list[i].clicked;
            } else {
                labs_list[i].clicked = false;
            }
        }
        this.setState({
            labs_list: labs_list
        ...
        <SafeAreaView style = {styles.container}>
        {
            ...
            <View style = {{width: '100%', height: main_view_height, alignItems: 'center'}} onStartShouldSetResponder={() => this.hidden_right_menu()}>
                <ScrollView style = {{width: '90%'}} showsVerticalScrollIndicator = {false}>
                    ...
                    {
                        this.state.labs_list.map((item, index) => 
                        <View key = {index} style = {{width: '100%', flexDirection: 'row', marginTop: 20, zIndex: 1000-index}}>
                            <View style = {{width: '80%'}}>
                                <Text style = {styles.component_content_text} multiline = {true}>{item.labDesc}</Text>
                            </View>
                            <View style = {{width: '10%', alignItems: 'center'}}>
                                <View style = {[{width: 20, height: 20, borderRadius: 20, justifyContent: 'center', alignItems: 'center'}, item.labStatus == "P" ? {backgroundColor: '#1c88e5'} : {backgroundColor: '#f7a722'}]} >
                                    <Text style = {{fontSize: 16, color: '#ffffff'}}>{item.labStatus}</Text>
                                </View>
                            </View>
                            ...
                            </View>
                            {
                                ...
                                <View style = {[styles.right_menu_view, {zIndex: 1000-index}]}>
                                    ...
                                    <TouchableOpacity style = {styles.right_menu_item} onPress = {() => this.show_alert(index)}>
                                        <Text style = {styles.right_menu_text}>Ordered</Text>
                                    </TouchableOpacity>
                                </View>
                            }
                        </View>
