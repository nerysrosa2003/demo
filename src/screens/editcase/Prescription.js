...
export default class Prescription extends Component {
    ...
    constructor(props){
		...
		this.state = {
            ...
            meds_list: Global.edit_case_json.meds,
    ...
    select_right_menu = (index) => {
        var meds_list = this.state.meds_list;
        for(i = 0; i < meds_list.length; i ++) {
            if(i == index) {
                meds_list[i].clicked = !meds_list[i].clicked;
            } else {
                meds_list[i].clicked = false;
            }
        }
        this.setState({
            meds_list: meds_list
    ...
    show_alert(index) {
        ...
        [
            ...
            {text: 'OK', onPress: () => this.delete_med(index)},
        ],
        { cancelable: true })
    }
        ...
        <SafeAreaView style = {styles.container}>
        {
            ...
            <View style = {{width: '100%', height: main_view_height, alignItems: 'center'}} onStartShouldSetResponder={() => this.hidden_right_menu()}>
                <ScrollView style = {{width: '90%'}} showsVerticalScrollIndicator = {false}>
                    ...
                    {
                        this.state.meds_list.map((item, index) => 
                        <View key = {index} style = {{width: '100%', flexDirection: 'row', marginTop: 20, zIndex: 1000-index}}>
                            <View style = {{width: '90%'}}>
                                <Text style = {styles.component_content_text} multiline = {true}>{item.medDesc}</Text>
                            </View>
