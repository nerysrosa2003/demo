...
export default class Alternatives extends Component {
    ...
    init_visit = () => {
        this.setState({
            ...
            alternatives_list: Global.edit_case_json.alternatives,
        })
    }
    ...
    select_right_menu = (index) => {
        var alternatives_list = this.state.alternatives_list;
        for(i = 0; i < alternatives_list.length; i ++) {
            if(i == index) {
                alternatives_list[i].clicked = !alternatives_list[i].clicked;
            } else {
                alternatives_list[i].clicked = false;
            }
        }
        this.setState({
    ...
    show_alert(index) {
        ...
        [
            ...
            {text: 'OK', onPress: () => this.delete_alternative(index)},
        ],
        { cancelable: true })
    }

    delete_alternative(index) {
        ...
        <SafeAreaView style = {styles.container}>
        {
            ...
            <View style = {{width: '100%', height: main_view_height, alignItems: 'center'}} onStartShouldSetResponder={() => this.hidden_right_menu()}>
                <ScrollView style = {{width: '90%'}} showsVerticalScrollIndicator = {false}>
                    ...
                    {
                        this.state.alternatives_list.map((item, index) => 
                        <View key = {index} style = {{width: '100%', flexDirection: 'row', marginTop: 20, zIndex: 1000-index}}>
                            <View style = {{width: '90%'}}>
                                <Text style = {styles.component_content_text} multiline = {true}>{item.alternativeDesc}</Text>
                            </View>
